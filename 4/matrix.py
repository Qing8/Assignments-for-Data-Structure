...
class Matrix :
    ...
    def max_value(self):
        # To do ......
        max_value = self._theGrid._theRows[0][0]
        for i in self._theGrid._theRows:
            for j in i:
                if j > max_value:
                    max_value = j
        return max_value
...
#                self.__setitem__((row,column),scalar*self.__getitem__((row,column)))
    ...
    def transpose( self ):
        # To do ......
################# self.new = Array2D(self.numCols(),self.numRows())
        self.new = Matrix(self.numCols(),self.numRows())
        for row in range(self.numRows()):
            for column in range(self.numCols()):
                self.new[column,row] = self[row,column]
        return self.new
    
    # Creates and returns a new matrix that results from matrix addition.
    def __add__( self, rhsMatrix ):
        # To do ......
        self.new = Matrix(self.numRows(),self.numCols())
        for row in range(self.numRows()):
            for column in range(self.numCols()):
                self.new[row,column] = self[row,column] + rhsMatrix[row,column]              
        return self.new
    ...
    def __mul__( self, rhsMatrix ):
        # To do ......
        self.new = Matrix(self.numRows(),rhsMatrix.numCols())
        for row in range(self.numRows()):
            for column in range(rhsMatrix.numCols()):
                for i in range(rhsMatrix.numRows()):
#                    print("self.new[row,column]",self.new[row,column])
#                    print("self[row,i]",self[row,i])
#                    print("rhsMatrix[i,column]",rhsMatrix[i,column])
                    self.new[row,column] += self[row,i] * rhsMatrix[i,column]
        return self.new
